...
latest_records AS (
    ...
    FROM (
        ...
        FROM `dbtvault`.`satellite_ghost_incremental` AS current_records
            JOIN (
...
ghost AS (
    ...
    NULL AS MKTSEGMENT,
    NULL AS COMMENT,
    TO_TIMESTAMP('1900-01-01 00:00:00') AS LOAD_DATETIME,
    TO_TIMESTAMP('1900-01-01 00:00:00') AS EFFECTIVE_FROM,
...
records_to_insert AS (
    SELECT
        ...
        FROM ghost AS g
        WHERE NOT EXISTS ( SELECT 1 FROM `dbtvault`.`satellite_ghost_incremental` AS h WHERE h.HASHDIFF = g.HASHDIFF )
    UNION 
